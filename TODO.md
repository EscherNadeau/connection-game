...
## 29. The Reveal + the Credits rename ✅ DONE (2026-07-11)
Escher's ask: a post-game mode — see the game you just played, click into anything, walk credits, expand the web, find alternate routes, see a best path, then share a framed image of the board. Also: rename Knowledge mode.
- [x] **"Knowledge" → "Credits"** (UI only — home switcher "Play Credits", scene-editor chip; internal id, blob `m:"k"`, and every function name stay `knowledge`). You're reciting the credits — the name now says what the round is.
- [x] **The Reveal:** entering "view the web" after a finish flips `revealMode` (not on the finale board — namespaced island keys — and not in party). The peek modal grows a **walkable credits grid** (`renderPeekConns` off `loadDetail`'s cached conns; panel widens + poster shrinks via `.peek-panel.reveal`); tapping a credit `expandReveal`s it onto the board — or just draws the missing edge if both ends are already there. **Deliberately no ledger/stat/record writes anywhere in the reveal** — it's browsing the answer sheet, not playing (same honesty rule as practice days).
- [x] **🥇 tightest route:** BFS on the current web (per-goal for double features), lit as sage-dashed `.alt-path` alongside the gold win path; message compares it to your route ("2 shorter than your 6"). Expanding the web then re-pressing finds new shortcuts — alternate routes emerge from exploration instead of a solver. Hidden in Credits mode (no goal). `game.winPath`/`game.winPaths` captured at win time (reset in every start fn — a stale hybrid path must never leak into a classic compare).
- [x] **🖼 frame the web:** the whole board as a one-sheet PNG (`drawBoardFrame` → download): warm paper + double frame (`cardPaper`), marquee title ("Ghostbusters → Fargo" in Instrument italic, shrink-to-fit), every node a poster tile (reuses `cardPoster`, labels allowed 1.7× tile width — a `labelW` param added), gold win-path edges over plain ones, sage/clay/gold rings, fine-print footer. w342 posters on purpose (`loadCardImg` grew a `hi` flag) — a 30-node web would pull 30 w500s otherwise.
- Verified end-to-end in headless Firefox against live TMDB: real credits in the reveal peek (on-web items dimmed + labeled), framed PNG rendered pixel-perfect.
- **Gotcha found the day it shipped (2026-07-11, Escher's console):** `checkWin`'s card subtitle read bare `rules?.title` — a ReferenceError on every NON-daily classic win since #24 shipped it (2026-07-08). Nobody saw it because `tryPlace`'s catch swallowed it as "Something went wrong talking to TMDB" — the win modal just never opened. Fixed to `game.rules?.title`. Two lessons: bare identifiers in rarely-hit branches survive `node --check` (it's a runtime error), and that broad catch can mask real bugs as network noise — worth narrowing someday.
- Later: reveal from the LOSE modal ("see what you missed" — needs a view-web button there); search-to-expand during the reveal (search is dead post-game today); a "best possible path" beyond the visible web would need a TMDB BFS solver — decide if the honesty is worth the spoiler power.

## 28. The party pass, first cut ✅ DONE (2026-07-11)
The Tier-0 / #25 merge, built as one presentation pass (they're the same muscle):
- [x] **Big flares:** `bigFlare()` — a full-screen overlay (own dark veil so it reads in BOTH themes; `pointer-events: none` so input is never blocked; a new flare replaces a mid-flight one instead of queueing; reduced-motion = fade only). Fires from `tryPlace` (solo, all modes) and the Blitz stage (with the finder's name via `.flare-by`). The toast-line flare stays as the quiet echo. Every link is person↔title by the one rule, so the credit always reads "PERSON *was in* TITLE".
- [x] **Reveal-first win (classic):** when the winning chain holds a deep-cut/crazy link, the pull IS the headline ("Ramis *was in Airheads.*", eyebrow "The pull 🤯"), route second, stars demoted to fine print BELOW the route (moved in the DOM; kept every plumbing job — medals, share text, par). Pull-less wins keep "Connected." — the override runs *after* the shell-restore lines, so the knowledge/hybrid/finale contract is untouched. The save-card headline carries the pull too (the card is the souvenir — full reveal deliberate, per the three-surfaces rule). Hybrid wins keep their old headline for now (a multi-path reveal is its own design).
- [x] **Instant next round:** quick-play win AND loss modals now roll a fresh bill straight onto the board (`nextRound()` — classic / knowledge / double feature), zero navigation. Setup slots + the DF deck stay in sync so a later quit shows the matchup you actually played. The button reads "Next Round →" in quick play only (`syncPlayAgainLabel()` at all four win surfaces); daily / quest / builder exits unchanged. Pool failure falls back to the setup screen — never a dead button.
- Deliberately NOT in this cut: couch-scale board text / a "party display" toggle (decide on a real couch first), the share-text tempt line (#25, still open).

## 27. Production hardening pass ✅ DONE (2026-07-11)
Code-level "production ready" sweep; the app-level review that followed it set the order for #28.
- [x] **Storage armor:** every localStorage touch rides `lsGet`/`lsSet`/`lsJSON` (top of app.js). Blocked storage (cookies-off), full/private-mode writes, and corrupt JSON all degrade to "plays fine, doesn't persist" (one console warn) instead of crashing whatever flow was mid-write — the daily win record was the scariest casualty. `npm test` now FAILS on any raw `localStorage` call so the guard can't erode.
- [x] **Last-resort net:** `window` error + unhandledrejection → console + a one-time plain-language pill (`.crash-note`, styled for both themes, above every modal). Plain on purpose — errors are never themed.
- [x] **CI:** `check.mjs` also fails on duplicate ids in index.html (the #19 podium/intermission cross-wiring gotcha, now caught by machine).
- Launch checklist still on Escher's side: rotate the leaked Supabase secret, `{{ .Token }}` in the Magic Link template, Site URL, verify a real domain in Resend.

## 26. Studio roadshows — the soft scene cap ✅ DONE (2026-07-08)
Escher: features should be allowed to run longer than 5 scenes. Decided as **soft 5 / hard 12**: `SOFT_SCENES`=5 stays the marquee length (past it, the add button reads "＋ add a scene — roadshow territory" — the old-Hollywood word for features long enough to need intermissions, which these literally have); `MAX_SCENES`=12 is the hard rail for the URL blob and the finale board. Verified N-agnostic before raising: finale island grid (`√n` cols), scene colors (cycle), poster fan (slices to 6), par arrays. **Gotcha fixed en route: `loadFeature` was hard-slicing incoming links to 5 scenes** — a longer feature would have silently lost its ending; the slice now follows `MAX_SCENES` (keep them in lockstep). Lens check: bigger custom nights for the huddle — pure (a).

## 25. The Reveal pass (SHAPING 2026-07-08 — backlog from an outside review of GAME.md)
The review's verdict, adopted as permanent principle in GAME.md ("the lens"): **surprise is the product**; the engine already measures it (`edgeTier`) but mumbles where it should shout. Principles live in GAME.md; this is the implementation backlog they produced — kept separate on purpose (principles say what the game IS; these are just work):
- [x] **Reveal-first win screen** — built 2026-07-11, see #28 (classic only; hybrid's multi-path reveal deferred).
- [ ] **Share text = tempt, never spoil** (re-affirms #18's spoiler-free rule, which a reveal-first share would have broken): tier + link count + a challenge link ("your turn"). The lobby card stays the souvenir — full route, explicit extra tap, never the default.
- [x] **Flares get BIG** — built 2026-07-11, see #28.
- [ ] **Audit mid-game surfaces for fame-tier leaks:** in the flow of play, tiers must only ever appear *after* placement (discovery: "oh wow, that was crazy"), never before (strategy: "I need crazy"). Check the in-game autocomplete never shows fame badges. Setup surfaces (casting page) and the party stage (deep cuts pay triple) keep theirs deliberately — priming deep memory IS the fun there.
- [ ] **Route Journal — the seed of the detailed-Archive conversation (not scheduled):** solved routes kept automatically, nameable, shareable — the *earned* twin of the free-pick Four Favorites that #24 built and cut the same day. "Every connection, kept," made literal. Most machinery exists (win-path capture via `lastCard`, stubs). Design it when the Archive conversation happens; it belongs there, never on the profile.
...
- **Deep cuts / crazy pulls = DISTINCT links found** (a Set of link ids), not raw flare counts — replays and undo-cycling never double-count, and "found" honestly means *discovered*. Captured in `tryPlace` at the flare site.
- **Bridges** = interior people on winning paths (`tallyPath` in classic `checkWin` + once per path in `hybridCheckWin`). **Your avatar IS your signature bridge** — their photo in the gold ring.
- **Rarest link** = tier first, ties broken by the obscure end's fame as a fraction of its type's "famous" bar (`obscurity()` — the cross-type comparison; thresholds mirror `fameTier`, keep in lockstep).
- **Solo modes only:** Opening Night never writes the ledger — the couch's collective finds aren't your record, and fake friends would pollute the books. Test screenings DO count (you really did route it).
...
- [x] **Four Favorite Connections — decided FREE PICK (Escher, 2026-07-08), built same day.** Search any two — a route you love (matches the mock's copy; nobody had a recorded solve history to "earn" from anyway). Rides the casting page: `cast.ctx {mode:"fav"}`, first corner → second corner → framed on the profile; ✕ on hover takes one down; localStorage `favorites`, cap 4. **v1 has no link-count chip** — computing a real route between free picks is a search problem, not a render; revisit if the cards feel naked (natural upgrade: stamp the count when you *play* a favorite as a matchup — the "earned badge later" fork).
- [x] **What's free vs. new** — shipped as predicted: streak/distribution straight off `dailyLog` (+ `dailyTier` reuse), the Ledger was the one small capture, favorites the only new UI.
- [x] Home entry: `🎞 your profile` ghost button; The Archive is one hop from the profile footer.
- [ ] Tier 3 stats (totals by mode, hint-free win rate, avg links) — need per-game records nobody keeps yet; design with Tier 4's schema.
- [ ] Tier 4 (The Charts, creator stats, Opening Night) — Supabase-era, unchanged.

## 23. Accounts + saved data (Supabase) — BUILT 2026-07-08, awaiting project credentials
"Live soon" needs data to survive a cleared cache / new device. Code shipped with #24; goes live the moment `SUPABASE_URL`/`SUPABASE_KEY` (top of the Accounts section in app.js) are pasted from Escher's dashboard.
- [x] **Auth = email OTP (decided 2026-07-08):** `signInWithOtp` → 6-digit `verifyOtp`, all inside the profile's account block (send → code → signed in). Zero external config; Google OAuth can be added later without UI rework. First sign-in creates the account.
- [x] **Sync scope (Escher, 2026-07-05):** `dailyLog`, `shelf`, `stubs`, the `ledger` tallies + `favorites`. **Settings explicitly OUT** — a per-sitting preference, not identity. One `profiles` row per user, one jsonb column per key.
- [x] **Local-first, enforced in the shape of the code:** localStorage is the only thing anything reads; Supabase is write-through backup (`queueSync` debounce piggybacked on every local save) + merge-on-sign-in (`mergeState`). supabase-js rides a CDN `<script>` — if it never loads or keys are blank, the account block politely says so and everything else is untouched.
- [x] **Merge rules (2026-07-08):** dailyLog per-date with local winning conflicts; shelf/stubs/favorites union deduped local-first keeping the caps (30/30/4); ledger sets union; **bridge counts take the MAX, never the sum** (both devices grew from the same synced base — summing double-counts); rarest keeps the rarer. Signing out loses nothing — the local reel keeps everything.
- [x] Project created + credentials wired (2026-07-08); profiles table + RLS confirmed live (anon probe returns `[]`). **Full loop verified same day:** send → ticket email → code → Signed in / synced ✓ → row in `profiles`. Note: this project issues 8-digit codes, so no UI copy may promise a length ("code from the email").
- [x] **Identity decisions (Escher, 2026-07-08):** **passwords NO, ever** — the email code is the account; if friction ever matters, add Google one-tap (same auth, no rework). **Handles = display names, later** — `profiles.handle` already exists and syncs (auto-filled from the email prefix today); becomes editable + unique when The Charts / online rooms need public names. **Online multiplayer** = Supabase Realtime on this same project, riding these same accounts (the #19 "Supabase era" plan — no separate server).
- [x] Custom SMTP live via Resend (`onboarding@resend.dev` for now — delivers only to Escher + lands in spam; **verify a real domain in Resend before launch**). Ticket-styled email template in `email-template.html` — paste into BOTH Magic Link AND Confirm signup (first-time sign-ins use Confirm signup — the gotcha that ate an hour).
- [ ] **Dashboard steps left (Escher):** Auth → Email Templates → Magic Link: add `{{ .Token }}` so the mail carries the 6-digit code → Auth → URL Configuration: Site URL = `https://eschernadeau.github.io/connection-game/` → rotate the secret key (pasted in chat 2026-07-08, treat as leaked).
- [ ] **Custom SMTP before real users:** Supabase's built-in mailer is dev-only — heavily rate-limited (a few auth emails/hour) and on new projects it only delivers to the project owner's own address. Fine for our testing; real sign-ups need an SMTP provider (Resend free tier integrates with Supabase in one screen). Not urgent until launch.
- [ ] Pairs with #21's "self-serve curated dailies" (fork c) and #19's rooms — same Supabase era. The Charts reads the `profiles` rows this ships.
